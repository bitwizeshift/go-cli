...
func AddToGroup(name string, flags ...*pflag.Flag) {
	annotation.AddToGroup(name, flags...)
}

const generalFlagsGroup = "General Flags"

// Groups returns a slice containing all flag [Group]s in the flagset, sorted
...
func Groups(fs *pflag.FlagSet) []*Group {
	...
	for _, g := range result {
		slices.SortFunc(g.Flags, func(lhs, rhs *pflag.Flag) int {
